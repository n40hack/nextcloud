  ...
  methods: {
    start() {
      ...
      picker.pick().then(dir => dir === '' ? '/' : dir).then(dir => {
        _logger_ts__WEBPACK_IMPORTED_MODULE_7__["default"].debug(`path ${dir} selected for transferring ownership`);
        if (!dir.startsWith('/')) {
        ...
        _logger_ts__WEBPACK_IMPORTED_MODULE_7__["default"].error(`Selecting object for transfer aborted: ${error.message || 'Unknown error'}`, {
          error
        });
    ...
    async findUser(query) {
      ...
      try {
        ...
        response.data.ocs.data.exact.users.concat(response.data.ocs.data.users).forEach(user => {
          vue__WEBPACK_IMPORTED_MODULE_5__["default"].set(this.userSuggestions, user.value.shareWith, {
        ...
        _logger_ts__WEBPACK_IMPORTED_MODULE_7__["default"].error('could not fetch users', {
          error
        });
      } finally {
    ...
    submit() {
      if (!this.canSubmit) {
        _logger_ts__WEBPACK_IMPORTED_MODULE_7__["default"].warn('ignoring form submit');
      }
      ...
      _logger_ts__WEBPACK_IMPORTED_MODULE_7__["default"].debug('submit transfer ownership form', data);
      ...
      _nextcloud_axios__WEBPACK_IMPORTED_MODULE_0__["default"].post(url, data).then(resp => resp.data).then(data => {
        _logger_ts__WEBPACK_IMPORTED_MODULE_7__["default"].info('Transfer ownership request sent', {
          data
        });
        ...
        _logger_ts__WEBPACK_IMPORTED_MODULE_7__["default"].error('Could not send ownership transfer request', {
          error
        });
...
/******/ })()
;
//# sourceMappingURL=files-settings-personal.js.map?v=0e69ab0a636c6a8a9ad4